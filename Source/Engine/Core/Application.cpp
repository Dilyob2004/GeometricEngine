...
namespace MeteorEngine
{
	...
	public:
		...
		void setupSurface()
		{
			...
			surfaceCreateInfo.hwnd = (HWND)window->GetWindowPtr();
		...
		void setupSwapchain()
		{
			...
			swapchainExtent.width = std::clamp<u32>(window->GetSize().x,
				surfaceCapabilities.minImageExtent.width,
				surfaceCapabilities.maxImageExtent.width);
			swapchainExtent.height = std::clamp<u32>(window->GetSize().y,
		...
		*/
		...
		void setupTextureImage()
		{
			...
			if (!imageData.Load("resources/logo.png"))
			{
				vulkanAvailable = false;
				return;
			}
			...
			std::memcpy(ptr, imageData.GetPointer(), static_cast<std::size_t>(imageSize));
