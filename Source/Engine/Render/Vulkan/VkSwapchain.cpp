...
namespace MeteorEngine
{
	...
	void VulkanSwapChain::InitSurface(void* platformWindow)
	{
		...
		if (graphicsQueueNodeIndex == UINT32_MAX || presentQueueNodeIndex == UINT32_MAX)
			LOG("Could not find a graphics and/or presenting queue!");

		// todo : Add support for separate graphics and presenting queue
		if (graphicsQueueNodeIndex != presentQueueNodeIndex)
			LOG("Separate graphics and presenting queues are not supported yet!");

		m_QueueNodeIndex = graphicsQueueNodeIndex;

		// Get list of supported surface formats
		uint32_t formatCount;
		err = vkGetPhysicalDeviceSurfaceFormatsKHR(m_PhysicalDevice, m_Surface, &formatCount, NULL);
		if (err != VK_SUCCESS)
		{
			LOG("Failed: vkGetPhysicalDeviceSurfaceFormatsKHR!\n");
			return;
		}
	...
	*/
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync, bool fullscreen)
	{
		...
		err = (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_PhysicalDevice, m_Surface, &surfCaps));
		if (err != VK_SUCCESS)
		{
			LOG("Failed: vkGetPhysicalDeviceSurfaceCapabilitiesKHR!\n");
			return;
		}
		// Get available present modes
		uint32_t presentModeCount;
		err = (vkGetPhysicalDeviceSurfacePresentModesKHR(m_PhysicalDevice, m_Surface, &presentModeCount, NULL));
		if (err != VK_SUCCESS)
		{
			LOG("Failed: vkGetPhysicalDeviceSurfacePresentModesKHR!\n");
			return;
		}
		...
		if (err != VK_SUCCESS)
		{
			LOG("Failed: vkCreateSwapchainKHR!\n");
			return;
		}
		...
		if (err != VK_SUCCESS)
		{
			LOG("Failed: vkGetSwapchainImagesKHR!\n");
			return;
		}
