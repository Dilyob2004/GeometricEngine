...
public:
	...
	{
		/*TArray<BufferElement> BufferElements;
		BufferElements.Push({ TEXT("POSITION"),	ShaderElementType::Float3 });
		BufferElements.Push({ TEXT("UV"),			ShaderElementType::Float2 });

		RHIPipelineStateDefinition PipelineDefinition;
		PipelineDefinition.VertexParameters = BufferElements;
		PipelineDefinition.PixelShader = GPSResourceMap[TEXT("PSFXAA")];
		PipelineDefinition.VertexShader = GVSResourceMap[TEXT("VSFXAA")];
		...
		TArray<Vertex2DData> VerticesData;
		...
		VertexBuffer = GDynamicRHI->RHICreateVertexBuffer(VerticesData.Pointer(), sizeof(Vertex2DData) * VerticesData.Count(), sizeof(Vertex2DData));
