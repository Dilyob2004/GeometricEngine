		...
		switch (Msg)
		{
			case WM_SIZE:
			{
				if (WParam != SIZE_MINIMIZED)
					OnResizeEvent(U32((int)(short)LOWORD(LParam)), U32((int)(short)HIWORD(LParam)));
				break;
			}
			case WM_CLOSE:
				OnClosedEvent();
...
WindowsKeyboard*		GPlatformKeyboard = NULL;
WindowsMouse*		GPlatformMouse = NULL;
WindowsEventWindow*	GPlatformEventWindow = NULL;
...
bool WindowsInput::OnResized()
{
	return GPlatformEventWindow->OnResized();
}
