...
DynamicRHI* DynamicRHI::CreateDynamicRHI()
{
	...
	TArray<DX11Adapter> AdapterList;
	...
	for(auto& i : AdapterList)
		if (i.Description.VendorId == GPU_VENDOR_ID_INTEL)
		{
			ChoosenAdapter = i;
			break;
		}
		else if (i.Description.VendorId == GPU_VENDOR_ID_NVIDIA)
		{
			ChoosenAdapter = i;
			break;
		}
		else if (i.Description.VendorId == GPU_VENDOR_ID_AMD)
		{
			ChoosenAdapter = i;
			break;
		}
...
bool DX11DynamicRHI::InitDevice()
{
	...
	if (FAILED(D3D11CreateDevice(EnumAdapter,
								...
								&DXDeviceContext)))
	{
		LOG("Error: [DX11RHI] Failed to Create a D3D Device!");
		return false;
	}
	return true;
}
