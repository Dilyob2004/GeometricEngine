...
RHITexture2D* DX11DynamicRHI::RHICreateTexture2D(const RHITextureDefinition& Definition, U8* Data)
{
	...
	return new DX11Texture2D(DXTexture, DXShaderResourceView, Definition);
}


RHIRenderTarget* DX11DynamicRHI::RHICreateRenderTarget(const RHITextureDefinition& Definition)
{
	D3D11_TEXTURE2D_DESC Descriptor;
	SMemory::Zero(&Descriptor, sizeof(Descriptor));
	Descriptor.Width = Definition.Width;
	...
	return new DX11RenderTarget(DXTexture, DXRenderTargetView, DXShaderResourceView, Definition);
}


RHIDepthTarget* DX11DynamicRHI::RHICreateDepthTarget(const RHITextureDefinition& Definition)
{
	D3D11_TEXTURE2D_DESC Descriptor;
	SMemory::Zero(&Descriptor, sizeof(Descriptor));
	Descriptor.Width = Definition.Width;
	...
	Descriptor.Usage = EngineUsageToDX11(Definition.Usage);
	Descriptor.ArraySize = 1;
	Descriptor.BindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE;
	...
	return new DX11DepthTarget(DXTexture, DXShaderResourceView, DXDepthStencilView, Definition);
}