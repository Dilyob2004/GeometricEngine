...
static RHIRenderTarget* GetSwapChainBackBuffer(const DX11DynamicRHI* RHI, IDXGISwapChain* DXSwapChain)
{
	...
	return new DX11RenderTarget(BackBuffer, 
								DXRenderTargetView, 
...
void DX11DynamicRHI::RHIResizeViewport(RHIViewport* ViewportRHI, U32 SizeWidth, U32 SizeHeight, bool EnableFullScreen)
{
	DYNAMIC_CAST(DX11Viewport, Viewport, ViewportRHI);
	Viewport->Resize(this, SizeWidth, SizeHeight, EnableFullScreen);
}
void DX11DynamicRHI::RHIBeginFrameViewport(RHIViewport* ViewportRHI)
{
	DYNAMIC_CAST(DX11Viewport, Viewport, ViewportRHI);
	RHIClearRenderTarget(Viewport->GetBackBufferView(), 30, 30, 30, 30);
	RHISetRenderTarget(Viewport->GetBackBufferView());
}	
void DX11DynamicRHI::RHIBeginFrameViewportDepth(RHIViewport* ViewportRHI, RHIDepthTarget* DepthTHI)
{
	DYNAMIC_CAST(DX11Viewport, Viewport, ViewportRHI);
	RHIClearRenderTarget(Viewport->GetBackBufferView(), 30, 30, 30, 30);
...
void DX11DynamicRHI::RHIEndFrameViewport(RHIViewport* ViewportRHI)
{
	DYNAMIC_CAST(DX11Viewport, Viewport, ViewportRHI);
	Viewport->Present();
}
RHIRenderTarget* DX11DynamicRHI::RHIGetViewportBackBuffer(const RHIViewport* ViewportRHI)
{
	DYNAMIC_CAST(DX11Viewport, Viewport, ViewportRHI);
